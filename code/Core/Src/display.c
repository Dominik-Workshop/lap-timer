...
static inline void write_cmd(const uint8_t byte) {
	HAL_I2C_Mem_Write(&hi2c3, DISPLAY_ADDRESS << 1, 0x00, 1, (uint8_t *) &byte, 1, HAL_MAX_DELAY);
}

static inline void write_buffer(const void *src, const uint32_t size) {
	HAL_I2C_Mem_Write(&hi2c3, DISPLAY_ADDRESS << 1, 0x40, 1, (uint8_t *) src, size, HAL_MAX_DELAY);
}
